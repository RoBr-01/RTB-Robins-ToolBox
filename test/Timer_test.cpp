...
TEST_F(TimerTest, MicrosecondsResolution) {
    ...
    float elapsed_us = timer.StopAndCount("us");
...
TEST_F(TimerTest, InvalidResolutionReturnsNanoseconds) {
    ...
    float elapsed_invalid = timer.StopAndCount("invalid");
    float elapsed_ns = timer.StopAndCount("ns");
...
TEST_F(TimerTest, MultipleStopAndCountCalls) {
    ...
    float first_call = timer.StopAndCount("ms");
    std::this_thread::sleep_for(std::chrono::milliseconds(5));
    float second_call = timer.StopAndCount("ms");
...
TEST_F(TimerTest, ScopedTimerPrintsOnDestruction) {
    {
        ScopedTimer scoped_timer("TestOperation", "ms");
        std::this_thread::sleep_for(std::chrono::milliseconds(10));
...
TEST_F(TimerTest, ScopedTimerWithDifferentResolutions) {
    ClearOutput();
    {
        ScopedTimer scoped_timer("SecondsTest", "s");
        std::this_thread::sleep_for(std::chrono::milliseconds(50));
    }
...
TEST_F(TimerTest, ScopedTimerMeasuresCorrectTime) {
    ...
    {
        ScopedTimer scoped_timer("TimingTest", "ms");
...
TEST_F(TimerTest, MultipleScopedTimers) {
    ...
    {
        ScopedTimer timer1("First", "ms");
        std::this_thread::sleep_for(std::chrono::milliseconds(5));

        {
            ScopedTimer timer2("Second", "us");
            std::this_thread::sleep_for(std::chrono::milliseconds(5));
