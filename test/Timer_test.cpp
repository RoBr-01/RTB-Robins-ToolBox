...
constexpr double kMinSleepSlack = 0.80;  // measured >= 80% of requested sleep
constexpr double kMaxSleepSlack = 10.0;  // generous upper bound

// Relative comparison helper.
bool Near(double a, double b, double abs_tol = 1e-9, double rel_tol = 1e-6) {
...
ParsedTimerOutput ParseTimerOutput(const std::string& output) {
...
class TimerTest : public ::testing::Test {
   protected:
    ...
    static void ExpectElapsedWithin(double measured,
                                    double expected,
                                    double min_factor = kMinSleepSlack,
                                    double max_factor = kMaxSleepSlack) {
...
TEST(TimerDetail, DurationLabelMappings) {
    EXPECT_STREQ(detail::durationLabel<std::chrono::seconds>(), "s");
    EXPECT_STREQ(detail::durationLabel<std::chrono::milliseconds>(), "ms");
    EXPECT_STREQ(detail::durationLabel<std::chrono::microseconds>(), "us");
    EXPECT_STREQ(detail::durationLabel<std::chrono::nanoseconds>(), "ns");

    // Unmapped durations default to "ns".
    EXPECT_STREQ(detail::durationLabel<std::chrono::minutes>(), "ns");
}
...
TEST_F(TimerTest, PrintWithoutLabel) {
    ...
    const auto parsed = ParseTimerOutput(Output());
...
TEST_F(TimerTest, ScopedTimerPrintsOnDestruction) {
    {
        ScopedTimer<std::chrono::milliseconds> timer("ScopedOp");
        SleepFor(std::chrono::milliseconds(10));
    }
...
TEST_F(TimerTest, NestedScopedTimersPrintInReverseDestructionOrder) {
    {
        ScopedTimer<std::chrono::milliseconds> outer("Outer");
        SleepFor(std::chrono::milliseconds(2));

        {
            ScopedTimer<std::chrono::microseconds> inner("Inner");
            SleepFor(std::chrono::milliseconds(2));
        }
...
// -----------------------------------------------------------------------------
// Numerical sanity of ScopedTimer output
// -----------------------------------------------------------------------------

TEST_F(TimerTest, ScopedTimerMeasuresExpectedDuration) {
    {
        ScopedTimer<std::chrono::milliseconds> timer("Measured");
