...
class Vector {
 ...
 public:
  Vector() { components.fill(static_cast<T>(0)); }
  ...
  Vector operator-() const {
    ...
    std::transform(components.begin(), components.end(),
                   result.components.begin(), [](T x) { return -x; });
    return result;
  }
  ...
  Vector &operator/=(const T t) { return *this *= static_cast<T>(1) / t; }
...
auto dotprod(const Vector<T, N> &v1, const Vector<T2, N> &v2) ->
  ...
  return static_cast<ResultType>(v1[0] * v2[0] + v1[1] * v2[1] + v1[2] * v2[2]);
}
